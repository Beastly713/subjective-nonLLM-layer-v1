...
function allRequiredAnswersPresent(answers: object) {
  return ['U1', 'R1', 'R2', 'R3', 'R4', 'R5', 'P1', 'P2', 'P3', 'P4', 'P5'].every(
...
function responseRows(answers: object) {
  ...
  return items.flatMap(([itemId, itemKey]) => {
    if (!Object.prototype.hasOwnProperty.call(values, itemId)) return [];
...
function reductionConflict(
  ...
  const allKnownZero = days.length === 7 && days.every((day) => day.status === 'KNOWN_ZERO');
  return (answers.U1 === false && hasPositiveQuantity) ||
...
function enforcePatientSafety(
  ...
  if (safety.requiresSafetyShell || safety.monitoringPromptPolicy === 'PAUSE') {
    throw new DomainError(409, 'SAFETY_PAUSED', 'Weekly check-ins are paused while the safety handoff is active.');
  }
  if (safety.reassessmentDueAt && now >= new Date(safety.reassessmentDueAt)) {
    throw new DomainError(409, 'SAFETY_REASSESSMENT_REQUIRED', 'Complete the required safety reassessment before continuing.');
  }
}
...
export async function correctWeeklyAssessment(input: {
  ...
  patientId: string;
  assessmentId: string;
  request: WeeklyAssessmentCorrectionRequest | StaffWeeklyAssessmentCorrectionRequest;
  requestId: string;
  actorId: string;
  actorType: 'PATIENT' | 'CLINICIAN';
}): Promise<CheckInStateResponse | CheckInMutationReceipt> {
  const { tx, clock, patientId, assessmentId, request, requestId, actorId, actorType } = input;
  if (actorType === 'CLINICIAN') {
    const assignment = await tx.clinicianPatientAssignment.findFirst({
      where: { clinicianUserId: actorId, patientId, endedAt: null },
      select: { id: true },
    });
  ...
  const assessment = await tx.weeklyAssessment.findFirst({
    ...
    include: {
      scheduledPeriod: { include: { scheduleVersion: { select: { version: true } } } },
      authoritativeRevision: { include: { itemResponses: true } },
    },
  });
  if (!assessment || !assessment.authoritativeRevision) notFound();
  ...
  if (
    ...
    throw new DomainError(409, 'VERSION_CONFLICT', 'The authoritative check-in changed before this correction.');
  }
  ...
  if (actorType === 'PATIENT') enforcePatientSafety(safety, now);
  ...
  if (request.completionIntent === 'COMPLETE' && !allAnswered) {
    throw new DomainError(400, 'COMPLETE_REQUIRES_ALL_ITEMS', 'Complete submission requires an answer for every weekly check-in item.');
  }
  if (request.completionIntent === 'PARTIAL' && allAnswered) {
    throw new DomainError(400, 'INCONSISTENT_COMPLETION_INTENT', 'All answered items require a COMPLETE correction intent.');
  }
  const days = WeeklyAlcoholDayInputSetSchema.parse(request.weeklyConsumptionDays ?? []);
  const expectedDates = new Set(periodLocalDates(period));
  if (days.some((day) => !expectedDates.has(day.localDate))) {
    throw new DomainError(400, 'VALIDATION_ERROR', 'Every weekly consumption day must belong to the persisted assessment period.');
  }
  if (goal?.goal !== 'REDUCTION' && days.length > 0) {
    throw new DomainError(400, 'VALIDATION_ERROR', 'Weekly consumption days are only available for a period-effective reduction goal.');
  }
  const historical = await loadHistoricalWeeklyObservations(tx, patientId, period.periodStartAt);
  ...
  if (goal?.goal === 'REDUCTION') {
    if (!goal.baselineRevisionId) throw new DomainError(409, 'REDUCTION_BASELINE_REQUIRED', 'A confirmed reduction baseline is required before this correction can be submitted.');
    baseline = await tx.reductionBaselineRevision.findUnique({
      where: { id: goal.baselineRevisionId },
      include: { days: { orderBy: { localDate: 'asc' } } },
    });
    if (!baseline || baseline.lifecycle !== 'CONFIRMED') throw new DomainError(409, 'REDUCTION_BASELINE_REQUIRED', 'The period-effective reduction baseline is not confirmed.');
    finalizedReduction = finalizeReductionWeek({
      dates: periodLocalDates(period),
      periodStartAt: period.periodStartAt,
      draftDays: days,
      targetWeeklyStandardDrinks: goal.targetWeeklyStandardDrinks === null ? null : Number(goal.targetWeeklyStandardDrinks),
      baselineAverageWeeklyDrinks: goal.baselineAverageWeeklyDrinks === null ? null : Number(goal.baselineAverageWeeklyDrinks),
    ...
    if (reductionConflict(answers, finalizedReduction.days)) {
      throw new DomainError(400, 'WEEKLY_ALCOHOL_CONFLICT', 'The weekly alcohol answer conflicts with one or more calendar entries. Correct either source before submitting.');
    }
  }

  await revokeEvaluationsForRevision(tx, previousRevision.id, now);
  const classification = actorType === 'PATIENT' ? 'PATIENT_CORRECTION' : 'STAFF_CORRECTION';
  const revision = await tx.assessmentRevision.create({
    data: {
      ...
      provenance: {
        source: actorType === 'PATIENT' ? 'PATIENT_CORRECTION' : 'STAFF_CORRECTION',
        actorId,
        previousRevisionId: previousRevision.id,
        periodId: period.id,
        trigger: actorType === 'PATIENT' ? 'CURRENT_PATIENT_CORRECTION' : 'STAFF_CORRECTION',
        ...(actorType === 'CLINICIAN' ? { reason: (request as StaffWeeklyAssessmentCorrectionRequest).reason } : {}),
      },
    },
  });
  const rows = responseRows(answers).map((row) => ({ ...row, assessmentRevisionId: revision.id }));
  if (rows.length > 0) await tx.assessmentItemResponse.createMany({ data: rows });
  await tx.useObservationLedger.create({
    data: {
      ...
      status: answers.U1 === true ? 'POSITIVE' : answers.U1 === false ? 'NEGATIVE' : 'UNKNOWN',
      observedAt: now,
      provenance: { itemId: 'U1', instrumentVersion: AUD_WEEKLY_CHECKIN_INSTRUMENT_VERSION },
      evaluationId: null,
    },
  });
  ...
  const recomputation = await recomputePatientMonitoringFromPeriod({
    ...
    patientId,
    periodId: period.id,
    authoritativeTrigger: actorType === 'PATIENT' ? 'CURRENT_PATIENT_CORRECTION' : 'STAFF_CORRECTION',
    actorId,
    requestId,
  });
  await tx.auditEvent.create({
    data: {
      ...
      action: actorType === 'PATIENT' ? 'WEEKLY_ASSESSMENT_CORRECTED' : 'WEEKLY_ASSESSMENT_STAFF_CORRECTED',
      entityType: 'ASSESSMENT_REVISION',
      entityId: revision.id,
      patientId,
      requestId,
      reason: actorType === 'CLINICIAN' ? (request as StaffWeeklyAssessmentCorrectionRequest).reason : null,
      ruleSetVersion: SUBJECTIVE_MONITORING_V1.ruleSetVersion,
      instrumentVersion: AUD_WEEKLY_CHECKIN_INSTRUMENT_VERSION,
      configurationVersion: SUBJECTIVE_MONITORING_V1.configurationVersion,
