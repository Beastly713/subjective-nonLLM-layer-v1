...
} from '../monitoring/domain/evaluate-weekly-assessment.js';
import {
...
async function refreshReductionSummary(input: {
  ...
  if (!input.goal.baselineRevisionId) return input.summary;
  const baseline = await input.tx.reductionBaselineRevision.findUnique({
    where: { id: input.goal.baselineRevisionId },
    include: { days: { orderBy: { localDate: 'asc' } } },
  });
  if (!baseline) return input.summary;
  ...
  if (input.summary) {
    await input.tx.weeklyConsumptionSummary.update({
      where: { assessmentRevisionId: input.revisionId },
      data,
    });
  } else {
...
export async function recomputePatientMonitoringFromPeriod(input: {
  ...
  const periods = await tx.scheduledPeriod.findMany({
    where: { patientId, periodStartAt: { gte: changedPeriod.periodStartAt } },
  ...
  const assessments = await tx.weeklyAssessment.findMany({
    where: {
      patientId,
      scheduledPeriodId: { in: allPeriods.map((period) => period.id) },
  ...
  const latestAuthoritativePeriodId = allPeriods
    .filter((period) => Boolean(assessmentByPeriod.get(period.id)?.authoritativeRevision))
    .at(-1)?.id ?? null;
  ...
  for (const period of periods) {
    ...
    if (persisted.created) createdEvaluationCount += 1;
    await tx.useObservationLedger.updateMany({
      where: { assessmentRevisionId: revision.id },
      data: { evaluationId: persisted.evaluation.id },
    });
    history.push({
      periodId: period.id,
      periodStartAt: period.periodStartAt,
  ...
  return { evaluationIds, createdEvaluationCount };
}
