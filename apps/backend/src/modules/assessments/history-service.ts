...
export async function startWeeklyAssessmentBackfill(
  ...
  const period = await tx.scheduledPeriod.findFirst({
    ...
    include: { scheduleVersion: { select: { version: true } } },
  });
  if (!period) notFound();
  if (now < period.openAt) {
    throw new DomainError(409, 'ASSESSMENT_PERIOD_NOT_OPEN', 'This historical check-in cannot be started before its scheduled opening.');
  }
  ...
  if (existing?.authoritativeRevision) {
    ...
    return projectCheckInState({
      ...
      assessment: existing,
      context: { period: periodRecord(period), goal, preference },
      safety,
      now,
    });
  }
  if (!(await hasNewerAuthoritativeAssessment(tx, patientId, periodRecord(period)))) {
    throw new DomainError(409, 'BACKFILL_NOT_AVAILABLE', 'A past check-in can be completed only after a newer weekly submission is authoritative.');
  }
  const [goal, preference, safety] = await Promise.all([
    resolveRecoveryGoalForPeriod(tx, patientId, period),
    resolvePreferencesForPeriod(tx, patientId, period),
  ...
  const assessment = existing ?? await tx.weeklyAssessment.create({
    data: {
      patientId,
      scheduledPeriodId: period.id,
      instrumentId: AUD_WEEKLY_CHECKIN_INSTRUMENT_ID,
    ...
    include: { authoritativeRevision: { select: { id: true, revisionNumber: true, completionStatus: true, submissionClassification: true, submittedAt: true, sourceDraftVersion: true } } },
  });
  return projectCheckInState({
    availability: 'HISTORICAL',
    period: periodRecord(period),
...
export async function readCheckInHistory(
  ...
  const assessments = await tx.weeklyAssessment.findMany({
    ...
    include: { revisions: { orderBy: { revisionNumber: 'desc' } }, authoritativeRevision: true },
  });
  const assessmentByPeriod = new Map(assessments.map((assessment) => [assessment.scheduledPeriodId, assessment]));
  const items = await Promise.all(periods.map(async (rawPeriod) => {
    ...
    return {
      ...
      submissionClassification: authoritative?.submissionClassification as
        | 'CURRENT'
        | 'LATE_CURRENT'
        | 'HISTORICAL_BACKFILL'
        | 'PATIENT_CORRECTION'
      ...
      revisions: assessment
        ? assessment.revisions.map((revision) => revisionProjection(revision, revision.id === authoritative?.id))
        : [],
      goal,
      preference,
    };
  ...
  const response = {
    items: items.map(({ goal: _goal, preference: _preference, ...item }) => item),
  };
  return CheckInHistoryResponseSchema.parse(response);
}
...
export async function readCheckInAssessmentDetail(
  ...
  const assessment = await tx.weeklyAssessment.findFirst({
    ...
    include: {
      ...
      revisions: {
        ...
        include: { itemResponses: true, alcoholConsumptionDays: { orderBy: { localDate: 'asc' } } },
      },
    },
  });
  ...
  const projectRevision = (revision: (typeof assessment.revisions)[number], isAuthoritative: boolean) => {
    const answers: Record<string, boolean | number> = {};
    for (const response of revision.itemResponses) {
      if (response.itemId === 'U1' && response.booleanValue !== null) answers.U1 = response.booleanValue;
      if (response.itemId !== 'U1' && response.integerValue !== null) answers[response.itemId] = response.integerValue;
    }
    return {
  ...
  const authoritative = assessment.revisions.find((revision) => revision.id === assessment.authoritativeRevisionId) ?? null;
