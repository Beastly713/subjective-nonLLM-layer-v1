...
export type ResolvedSafetyRoute = Awaited<ReturnType<typeof resolveSafetyRoute>>;
...
export async function evaluatePatientSafety(args: {
  ...
  const authoritative = await tx.onboardingRevision.findUniqueOrThrow({
    where: { id: onboarding.authoritativeRevisionId },
  });
  ...
  for (const domain of evaluation.domainResults) {
    const active = await tx.safetyCase.findFirst({
      ...
      include: {
        restrictions: { orderBy: { version: 'desc' }, take: 1 },
      },
    });

    if (!active) {
      const route = await resolveSafetyRoute(tx, args.config, evaluationNow, domain);
      ...
      await tx.safetyCase.update({
        ...
        data: { currentRestrictionVersionId: storedRestriction.id },
      });
      ...
      await appendLifecycle(tx, {
        ...
        fromState: null,
        toState: 'DETECTED',
        reason: 'Deterministic safety evaluation detected a controlled safety domain.',
    ...
    if (
      ...
      lifecycleReason = 'A new deterministic safety evaluation required emergency escalation.';
      ...
      lifecycleReason = 'Safety handoff lifecycle normalized after re-evaluation.';
    }
    ...
    if (severityTightens || gateTightens) {
      ...
      if (!sameRestriction(desiredRestriction, latestRestriction)) {
        ...
        await tx.safetyCase.update({
          where: { id: active.id },
          data: { currentRestrictionVersionId: restriction.id },
        });
      }
    }
  ...
  await tx.auditEvent.create({
    data: {
      ...
      metadata: { safetyEvaluationResultId: persistedEvaluation.id },
    },
  });
