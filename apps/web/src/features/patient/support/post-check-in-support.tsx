...
export function PostCheckInSupport() {
  ...
  if (!support || support.status !== 'AVAILABLE') return null;
  const resources = [support.primary, support.secondary].filter(
    (resource): resource is NonNullable<typeof support.primary> => Boolean(resource),
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <h2 className="mb-0 mt-2 text-xl font-semibold">A few optional next steps</h2>
        <p className="mb-0 mt-2 text-sm text-muted-foreground">
          These are based on the submitted check-in. Open Support to explore or share feedback.
        </p>
      </CardHeader>
      <CardContent className="grid gap-5">
        {resources.map((resource) => (
          <article className="grid gap-3 border-t pt-4 first:border-t-0 first:pt-0" key={resource.resourceId}>
